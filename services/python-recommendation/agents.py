...
class QueryAnalyzerAgent:
    ...
    SCENE_MARKERS = (
        'убил', 'убийство', 'утопил', 'утопила', 'процентщиц', 'ревизск', 'собак', 'барын',
        'души', 'застрелил', 'задушил', 'обманул',
    )
    THEME_MARKERS = (
        'порок', 'грех', 'вина', 'раская', 'человечеств', 'морал', 'нравствен', 'смысл',
        'обществен', 'лицемер', 'жесток', 'жадн',
    )
    SPECIFIC_PATTERNS = [
        r'\bв какой книге\b',
    ...
    def _determine_type(self, original_query: str, cleaned_query: str, keywords: List[str], title_matches: List[dict], query_analysis: dict):
        ...
        title_score = float(title_matches[0]['score']) if title_matches else 0.0
        strong_title = title_score >= 0.9
        soft_title = title_score >= 0.8
        short_title = title_score >= 0.72 and len(keywords) <= 3
        has_scene_markers = any(marker in lowered for marker in self.SCENE_MARKERS)
        has_theme_markers = any(marker in lowered for marker in self.THEME_MARKERS)
        has_feature_markers = any(query_analysis.get(flag) for flag in ('has_mood', 'has_style', 'has_plot', 'has_atmosphere', 'has_tone'))
        ...
        if any(re.search(pattern, lowered) for pattern in self.SPECIFIC_PATTERNS):
            return 'scene' if has_scene_markers else 'specific'

        if soft_title and len(keywords) <= 4 and not any(marker in lowered for marker in self.RECOMMENDATION_MARKERS):
            return 'title'

        if short_title and not has_scene_markers and not has_theme_markers and not any(marker in lowered for marker in self.RECOMMENDATION_MARKERS):
            return 'title'

        if has_scene_markers and len(keywords) >= 2:
            return 'scene'

        if has_theme_markers and any(marker in lowered for marker in self.RECOMMENDATION_MARKERS):
            return 'theme'

        if any(marker in lowered for marker in self.RECOMMENDATION_MARKERS):
            return 'theme' if has_theme_markers else 'recommendation'

        if has_feature_markers and len(keywords) >= 3:
            return 'mixed' if has_scene_markers or has_theme_markers else 'recommendation'

        if len(keywords) <= 2:
            return 'title' if title_score >= 0.72 else 'entity'

        if has_theme_markers:
            return 'theme'
    ...
    def analyze(self, query: str):
        ...
        if query_type in {'title', 'entity', 'specific', 'scene'}:
            priorities = {}
...
class ResponseAgent:
    ...
    def _result_response(self, results: List[dict], analysis: dict):
        response_type = analysis['type'] if analysis['type'] in {'title', 'entity', 'specific', 'scene', 'theme', 'mixed', 'keyword'} else 'general'
        payload = {
            'type': response_type,
            'query': analysis['original_query'],
            'query_type': analysis['type'],
    ...
    def format(self, results: List[dict], analysis: dict):
        ...
        if analysis['type'] in {'recommendation', 'theme', 'mixed'}:
...
class LLMPlannerAgent:
    ALLOWED_TYPES = {'title', 'entity', 'specific', 'scene', 'theme', 'mixed', 'keyword', 'recommendation'}
    ...
    def _build_prompt(self, query: str, analysis: dict) -> str:
        ...
        prompt = {
            ...
            'instruction': (
                ...
                'Допустимые query_type: title, entity, specific, scene, theme, mixed, keyword, recommendation. '
...
class SearchToolAgent:
    def _profile_for_query(self, tool_query: str, base_analysis: dict, planned_type: str) -> dict:
        ...
        is_recommendation = query_type in {'recommendation', 'theme', 'mixed'}
        return {
            'type': query_type,

...
from service_paths import RUNTIME_CONFIG_FILE
...
def estimate_runtime_requirements(config=None):
    ...
    if any(tag in model_hint for tag in ('14b', '15b')):
        full_ram = '24–48 ГБ RAM'
        full_vram = '16+ ГБ VRAM желательно'
    elif any(tag in model_hint for tag in ('7b', '8b', '9b', '13b')):
